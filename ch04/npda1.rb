...
class NPDARulebook < Struct.new(:rules)
  def applies_to?(configurations, character)
    !rule_for(configurations, character).nil?
  end
...
class NPDADesign < Struct.new(:start_state, :bottom_character,
  ...
  def accepts?(string)
    to_npda.tap { |dpda| dpda.read_string(string) }.accepting?
  end
